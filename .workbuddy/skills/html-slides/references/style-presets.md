...
Curated visual styles for HTML Slides. Each preset is inspired by real design references — no generic "AI slop" aesthetics. **Abstract shapes only — no illustrations.**

> **Note**: This is the initial framework. Style definitions will be expanded and refined based on user preference discussions. The current presets serve as a starting point.
...
- Consistent pill width, varying heights
...
CSS does not allow a leading `-` before function names. The browser silently discards the entire declaration — no error, just wrong positioning. **Always use `calc(-1 * ...)` to negate CSS function values.**
